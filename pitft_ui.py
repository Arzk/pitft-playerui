...
class PitftPlayerui:
	def __init__(self):
		...
		self.image["coverart_border_clean"] = pygame.image.load(self.path + "pics/" + "coverart-border.png")
		self.image["coverart_border_paused"] = pygame.image.load(self.path + "pics/" + "coverart-paused.png")
		self.image["coverart_border_play"] = pygame.image.load(self.path + "pics/" + "coverart-play.png")
		...
		if config.resolution[0] == 480 and config.resolution[1] == 320:
			...
			self.size["coverart"]             = 256
			self.size["selectorbutton"]       = 64
			...
			self.size["progressbar_height"]   = 16
		...
		try:
			...
			self.image["coverart_border_clean"]     = pygame.transform.scale(self.image["coverart_border_clean"], (self.size["coverart"], self.size["coverart"]))
			self.image["coverart_border_paused"]    = pygame.transform.scale(self.image["coverart_border_paused"], (self.size["coverart"], self.size["coverart"]))
			self.image["coverart_border_play"]      = pygame.transform.scale(self.image["coverart_border_play"], (self.size["coverart"], self.size["coverart"]))
		...
		# Cover art position
		self.pos["coverart"]      = config.resolution[0]/2 - self.size["coverart"]/2, self.pos["top"] #4

		# Area for track info
		self.pos["bottomdivider"] = self.pos["bottom"] - 78

		# Track information
		self.pos["track"]              = self.pos["left"] + self.size["elapsedmargin"], self.pos["bottom"] - self.size["trackinfo_height"]
		self.pos["album"]              = self.pos["track"][0], self.pos["track"][1] - self.size["trackinfo_height"]
		self.pos["progressbar"]        = self.pos["left"] + self.size["elapsed"] + 11, self.pos["album"][1] - 12 
		...
		self.pos["trackinfobackground"] = 0, self.pos["progressbar"][1] - 4
		...
		self.pos["progressbackground"] = 0, self.pos["progressbar"][1] - 4
		self.size["progressbackground"] = self.pos["track_length"][0], self.size["progressbar_height"] #427,20

		# Topmost selector button
		self.pos["selectorbuttonleft"] = self.pos["paddedleft"] + 12 , self.pos["paddedtop"] #418, 8
		self.pos["selectorbuttonright"] = self.pos["right"] - self.size["selectorbutton"], self.pos["paddedtop"] #418, 8

		# Center (pause) control button
		self.pos["controlbutton"] = self.pos["paddedright"] - self.size["controlbutton"] - 16, self.pos["paddedtop"] + 4 #418, 8

		# Selector buttons
		selector_buttons_shown = 0
		self.pos["button_spotify"]   = self.pos["selectorbuttonleft"] #418, 8
		...
		self.pos["button_playlists"] = self.pos["selectorbuttonleft"][0], self.pos["top"] + selector_buttons_shown * self.size["selectorbutton"] #418, 64
		if config.mpd_host and config.mpd_port:
			selector_buttons_shown = selector_buttons_shown + 1
		self.pos["button_cd"]        = self.pos["selectorbuttonleft"][0], self.pos["top"] + selector_buttons_shown * self.size["selectorbutton"] #418, 132
		if config.mpd_host and config.mpd_port and config.cdda_enabled:
			selector_buttons_shown = selector_buttons_shown + 1
		self.pos["button_radio"]     = self.pos["selectorbuttonleft"][0], self.pos["top"] + selector_buttons_shown * self.size["selectorbutton"] #418, 190

		# Control buttons
		self.pos["button_repeat"]        = self.pos["controlbutton"][0]                             , self.pos["controlbutton"][1] 
		self.pos["button_random"]        = self.pos["button_repeat"][0]                             , self.pos["button_repeat"][1] + self.size["togglebutton"][1]
		self.pos["icon_randomandrepeat"] = self.pos["button_repeat"][0] - 24                        , self.pos["button_repeat"][1] + 4 #285, 55
		self.pos["button_volumeminus"]   = self.pos["controlbutton"][0] - self.size["controlbutton"], self.pos["controlbutton"][1] + 2*self.size["controlbutton"] # 258, 190
		self.pos["button_volumeplus"]    = self.pos["controlbutton"][0] + self.size["controlbutton"], self.pos["controlbutton"][1] + 2*self.size["controlbutton"] # 354, 190
		...
		self.pos["list_right"]          = self.pos["selectorbuttonleft"][0] - 2 #416
	...
	def parse_song(self):
		...
		self.artist_album = self.artist + " - " + self.album
		...
		if self.updateAlbum or self.coverFetched:
			if self.cover:
				...
				if self.playbackStatus == "play":
					surface.blit(self.image["coverart_border_clean"],(self.pos["coverart"]))
					#self.updateAll
				else:
					surface.blit(self.image["coverart_border_paused"],(self.pos["coverart"]))
	#			surface.blit(self.image["coverart_border"],(self.pos["coverart"]))
		...
		if self.updateTrackInfo:
			...
			if self.date:
				text = self.font["details"].render(self.artist_album + " (" + self.date + ")", 1,(self.color["font"]))
			else:
				text = self.font["details"].render(self.artist_album, 1,(self.color["font"]))
...
#		if self.updateState:
	...
	def on_click(self, mousebutton, click_pos):
		...
		# Screen is on. Check which button is touched
			...
			# Controls
			elif self.pos["coverart"][0] <= click_pos[0] <= self.pos["coverart"][0] + self.size["coverart"] \
			and self.pos["coverart"][1] <= click_pos[1] <= self.pos["coverart"][1] + self.size["coverart"]:
				self.logger.debug("Toggle play/pause")
				self.button(7, mousebutton)			

			# Open playlist when longpressing on bottom
			elif self.pos["album"][1] <= click_pos[1] <= config.resolution[1] and mousebutton == 2:
